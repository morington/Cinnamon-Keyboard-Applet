...
class KbdLayoutController {
    constructor(instance_id) {
        this.settings = new Settings.AppletSettings(this, "keyboard@morington.org", instance_id);
        
        
        this.settings.bind("language-prefix", "LanguagePrefix", this.on_settings_changed);
        this.settings.bind("margin-width", "MarginWidth", this.on_settings_changed);
        this.settings.bind("margin-height", "MarginHeight", this.on_settings_changed);
    ...
    _on_settings_changed() {
        // this.LanguagePrefix = this.settings.getValue("language-prefix");
    }
    ...
    _on_layout_changed() {
        ...
        const layoutInfo = this.get_layout_data(this._current_layout_idx);
        const layoutName = layoutInfo.display_name || _("Unknown Layout");
        let message = `${this.LanguagePrefix}: ${layoutName}`;

        // debug
        // global.log(message);

        this.showMessageCenter(message);
    }

    showMessageCenter(message) {
        let messageBox = new St.Label({
            text: message,
        ...
        Main.uiGroup.add_actor(messageBox);

        let monitor = Main.layoutManager.primaryMonitor;
        messageBox.set_position(
            Math.floor(monitor.x + (monitor.width - messageBox.width) / 2 + this.MarginWidth),
            Math.floor(monitor.y + (monitor.height - messageBox.height) / 2 + this.MarginHeight)
        );
...
class CinnamonKeyboardApplet extends Applet.TextIconApplet {
    constructor(metadata, orientation, panel_height, instance_id) {
        ...
        try {
            ...
            this._controller = new KbdLayoutController(instance_id);
        }
        catch (e) {
            global.logError(e);
        }
